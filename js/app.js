// ===============================
// PeakCY — app.js (Hero Background Video + robust mobile menu)
// Paths: /js/app.js
// ===============================
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (toggle && panel) {
    ...
    // Re-adjust on resize if hash present (light debounce)
    let resizeTimer;
    ...
    console.warn("Menu toggle or mobile panel not found; skipping mobile menu wiring.");
  }
  ...
  if (heroVideo) {
    // Always keep inline & muted for mobile autoplay compliance
    heroVideo.setAttribute("playsinline", "");
    heroVideo.muted = true;
    heroVideo.loop = true;

    // If user prefers reduced motion, do not auto-play
    if (prefersReducedMotion) {
      try { heroVideo.pause(); } catch (_) {}
    }

    // IntersectionObserver to pause/resume when off-screen
    const io = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (prefersReducedMotion) return; // respect preference
